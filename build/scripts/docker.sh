...
if ! utils_command_exists docker; then
  ...
  *)
  ...
  cp /var/lib/sealer/data/my-cluster/rootfs/etc/daemon.json /etc/docker
  if [[ -n $1 && -n $2 ]]; then
    sed -i "s/sea.hub:5000/$2:$3/g" /var/lib/sealer/data/my-cluster/rootfs/etc/docker/daemon.json
  fi
fi
